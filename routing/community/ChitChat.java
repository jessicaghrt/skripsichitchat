 ...
 * @author IONE
 */
public class ChitChat implements RoutingDecisionEngine, TransientEngine {
    ...
    public void connectionUp(DTNHost thisHost, DTNHost peer) {
        ...
        ChitChat de = getOtherDecisionEngine(peer);
        if (this.trs.isEmpty()) {
    ...
    public void connectionDown(DTNHost thisHost, DTNHost peer) {
        ...
        ChitChat de = this.getOtherDecisionEngine(peer);
...
//        Tuple<Map<String, Double>, Map<String, Double>> historyTrs = new Tuple<>(trsAwal, trs);
//        Tuple<DTNHost, Tuple<Map<String, Double>, Map<String, Double>>> historyConn = new Tuple<>(peer, historyTrs);
//
//        trsInterval.add(historyConn);
    ...
    public boolean shouldSendMessageToHost(Message m, DTNHost otherHost, DTNHost thisHost) {
        ...
        ChitChat de = getOtherDecisionEngine(otherHost);
        for (String interest : this.trs.keySet()) {
            double trsNew = this.countTrsGrowth(thisHost, otherHost, interest);
            Map<String, Double> newTranscient = new HashMap<>(this.trs);
            newTranscient.put(interest, trsNew);
            this.trs = newTranscient;
        }
        for (String interest : de.trs.keySet()) {
            double trsNew = de.countTrsGrowth(otherHost, thisHost, interest);
            Map<String, Double> newTranscient = new HashMap<>(de.trs);
            newTranscient.put(interest, trsNew);
            de.trs = newTranscient;
        }
        ...
        double myTrs = this.trs.get(interest);
        double peerTrs = de.trs.get(interest);
        if (peerTrs > myTrs) {
            return true;
        }
        return false;
    ...
    private ChitChat getOtherDecisionEngine(DTNHost host) {
        MessageRouter otherRouter = host.getRouter();
        assert otherRouter instanceof DecisionEngineRouter : "This router only works "
                + " with other routers of same type";

        return (ChitChat) ((DecisionEngineRouter) otherRouter).getDecisionEngine();
    }
