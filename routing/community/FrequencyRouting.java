...
public class FrequencyRouting implements RoutingDecisionEngine {
...
//    private Map<DTNHost, Double> contactDur;
     ...
     */
    public FrequencyRouting(Settings s) {
     ...
     * object
     */
    public FrequencyRouting(FrequencyRouting proto) {
...
//        contactDur = new HashMap<DTNHost, Double>();
    ...
    public void doExchangeForNewConnection(Connection con, DTNHost peer) {
        ...
        FrequencyRouting de = this.getOtherDecisionEngine(peer);
...
//        }
    ...
    public boolean shouldSendMessageToHost(Message m, DTNHost otherHost, DTNHost thisHost) {
        ...
        FrequencyRouting de = this.getOtherDecisionEngine(otherHost);
        double myFreq = this.countFreq(m.getTo());
        double peerFreq = de.countFreq(m.getTo());
    ...
    public RoutingDecisionEngine replicate() {
        return new FrequencyRouting(this);
    }

    private FrequencyRouting getOtherDecisionEngine(DTNHost h) {
        MessageRouter otherRouter = h.getRouter();
        assert otherRouter instanceof DecisionEngineRouter : "This router only works "
                + " with other routers of same type";

        return (FrequencyRouting) ((DecisionEngineRouter) otherRouter).getDecisionEngine();
    }

    public int countFreq(DTNHost h) {
        if(this.connHistory.containsKey(h)){
            return this.connHistory.get(h).size();
        } else {
            return 0;
        }
    }
